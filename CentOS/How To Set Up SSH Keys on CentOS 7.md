...
```Linux Basics``` ```Security``` ```Getting Started``` ```System Tools``` ```CentOS```
...
You can then output the content you piped over into a file called authorized_keys within this directory. You’ll use the >> redirect symbol to append the content instead of overwriting it. This will let you add keys without destroying previously added keys.
...
You will manually append the content of your id_rsa.pub file to the ~/.ssh/authorized_keys file on your remote machine.
