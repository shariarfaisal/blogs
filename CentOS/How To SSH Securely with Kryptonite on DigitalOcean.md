...
```Security``` ```Ubuntu``` ```DigitalOcean``` ```Arch Linux``` ```CentOS``` ```CoreOS``` ```Debian``` ```FreeBSD``` ```Ubuntu 16.04```
...
The next step continues on your local computer. You’ll need to install the  kr command line utility, which enables SSH to authenticate with a key stored in Kryptonite. You can install kr with your preferred package manager (like npm or brew) or simply use curl, as we’ll do here.
...
You can find detailed instructions on adding your SSH key in Step 3 of this SSH on DigitalOcean tutorial.
...
For more information about how Kryptonite works, take a look at Kryptonite’s system architecture blog post and the Kryptonite and kr source code.
