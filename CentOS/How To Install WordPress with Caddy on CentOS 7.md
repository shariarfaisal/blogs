...
```Databases``` ```MySQL``` ```WordPress``` ```Let's Encrypt``` ```CentOS```
...
- MySQL installed by following the How To Install MySQL on CentOS 7 tutorial.
- Caddy installed by following the How To Host a Website with Caddy on CentOS 7 tutorial, including a domain name configured to point to your Droplet.
...
Open the PHP-FPM configuration file with vi or your favorite text editor. Here’s a short introduction to vi if you’re not familiar with it.
