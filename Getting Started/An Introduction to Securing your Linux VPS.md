...
```Linux Basics``` ```Getting Started``` ```FAQ``` ```Conceptual```


Status: Deprecated
This article is deprecated and no longer maintained. It may still be useful as a reference, but may not be up to date with the latest best practices.
See Instead
Check out our newer guide, Recommended Security Measures To Protect Your Servers.
...
You can learn how to implement a fail2ban policy on Ubuntu here.  There are similar guides for Debian and CentOS here.
