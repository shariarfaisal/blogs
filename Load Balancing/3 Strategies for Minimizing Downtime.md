...
```High Availability``` ```CI/CD``` ```Load Balancing``` ```Monitoring``` ```Conceptual```
...
- If all tests are passed, cut traffic over to the green environment by updating the load balancer config or by assigning the reserved IP to the green server.
...
## Using Reserved IPs and a Hot Spare Server
