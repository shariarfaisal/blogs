...
```Linux Basics``` ```Conceptual``` ```Open Source```
...
Shortly thereafter, the Open Source Initiative (OSI) was founded by Raymond and Bruce Perens to encourage both the use of the new term as well as the spread of open-source principles. OSI also developed the Open Source Definition—a list of ten principles which a software’s license must adhere to for it to be considered open-source:
