...
```Linux Basics``` ```Conceptual``` ```C (language)```
...
In this article, we have covered Linux’s roots in Unix and some of its defining features. If you are interested in learning more about the history of Linux and Unix variations (including FreeBSD), a good step might be our series on FreeBSD. Another option might be to consider our introductory series on getting started with Linux. You can also check out this introduction to the filesystem layout in Linux, this discussion of how to use find and locate to search for files on a Linux VPS, or this introduction to regular expressions on the command line.
